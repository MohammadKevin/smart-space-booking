...
export function PaymentStatusBadge({ status, className = "" }: PaymentStatusBadgeProps) {
  ...
  switch (normalized) {
    case "belum_bayar":
      return (
        <span
          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-xs text-[11px] font-semibold bg-slate-100 text-slate-600 border border-slate-200 ${className}`}
        >
    ...
    case "refund":
      return (
        <span
          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-xs text-[11px] font-semibold bg-indigo-50 text-indigo-800 border border-indigo-200 ${className}`}
        >
