...
export default function MemberProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("Profil akun member berhasil diperbarui.");
      setOldPassword("");
      setNewPassword("");
      setConfirmPassword("");
    } catch (err: unknown) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-2xl">
            Perbarui data diri, nomor kontak, foto profil check-in, dan kata sandi akun Anda.
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-xs p-6 shadow-2xs space-y-5">
              <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Data identitas resmi pemegang tiket reservasi ruang kerja.
                  </p>
                </div>
              ...
              <div className="space-y-2">
                <label className="block text-xs font-bold text-slate-700">Foto Profil / Avatar</label>
                <ImageUploader
                  value={foto}
                  onChange={(val) => setFoto(val || "")}
                  label=""
                  helperText="Upload foto profil resmi untuk verifikasi saat check-in tiket."
                />
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xs p-6 shadow-2xs space-y-5">
              <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Perbarui kata sandi akun untuk menjaga keamanan reservasi Anda.
                  </p>
                </div>
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center justify-between py-1 border-b border-slate-100">
                ...
                <span className="font-mono font-bold text-slate-800">
                  #MBR-{String(user?.member?.id || user?.id || 1).padStart(3, "0")}
                </span>
              </div>
              <div className="flex items-center justify-between py-1">
                <span className="text-slate-500">Instansi</span>
                <span className="font-medium text-slate-800 truncate max-w-[150px]">{instansi || "Umum"}</span>
              </div>
            </div>
          </div>
